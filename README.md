...
5. **对照试验**（`bench_order`）—— 一张开台单同时挂住**一台仪器 + 一瓶对照试剂**，
   柜型配对也得过关，缺一样台都开不出去：
   - 柜型 × 仪器两条红线硬拦：通风柜试剂 × 分光光度计（挥发搅光路）、
     防爆柜试剂 × 恒温水浴（加热有引爆风险）；仪器维修中 / 停用同样开不了
   - 开台即从现库存**预占**对照试剂 —— 只动库存，**不写领用 / 退回流水**
   - 同一台仪器上未收口的单只许一条（数据库唯一索引兜底，并发也拦得住）
   - 完成收口：预占转消耗不吐回；仪器中途被改成维修中 / 停用的单**只能整单作废**，
     不许点完成，也没有换试剂硬收口的口子
   - 整单作废：预占库存吐回，试剂回到还能再开的状态
