...
5. **对照开台**（`bench_order`）—— 一张开台单同时挂一台仪器和一瓶对照试剂：
   - **柜型配对**和**仪器状态**两道关一道都不能少：通风柜的试剂配不了分光光度计（挥发搅光路），
     防爆柜的试剂配不了恒温水浴（加热有引爆风险）；仪器维修中、停用同样开不出去
   - 开台即从现库存**预占**一瓶（不写领用/退回流水），收口才真正销掉
   - 中途失败、或仪器被改成维修中/停用：未收口的单**只能整单作废**，预占吐回，不许点完成、也不许换试剂硬收口
   - 同一台仪器未收口的开台只许一条（数据库唯一索引兜底）
